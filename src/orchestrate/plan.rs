...
pub struct AgentTask {
    /// Agent name (must be declared as an owner in repo.bog).
    pub agent: String,
...
pub fn validate_plan(plan: &DockPlan, ctx: &RepoContext) -> Result<(), OrchestrateError> {
    for (i, task) in plan.tasks.iter().enumerate() {
        // Check agent exists in repo.bog declarations
        if !ctx.derived_agents.roles.contains_key(&task.agent) {
            return Err(OrchestrateError::InvalidPlan(format!(
                "Task {i}: agent '{}' is not declared as an owner in repo.bog",
                task.agent
            )));
        }
