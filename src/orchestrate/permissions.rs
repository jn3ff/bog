    ...
    match role {
        ...
        None => {
            for entry in diff_entries {
                violations.push(Violation {
                    file_path: entry.path.clone(),
                    reason: format!("Agent '{agent_name}' is not declared as owner in repo.bog"),
                });
            }
        }
    }
...
mod tests {
    use super::*;
    use super::super::worktree::DiffChangeType;
    ...
    fn test_subsystem_agent_allowed_files() {
        let ctx = load_ctx();
        let entries = vec![diff("src/ast.rs"), diff("src/parser.rs")];
    ...
    fn test_subsystem_agent_disallowed_files() {
        let ctx = load_ctx();
        let entries = vec![diff("src/cli.rs")];
        let violations = check_agent_permissions("core-agent", &entries, &ctx);
    ...
    fn test_skimsystem_agent_allowed_bog_files() {
        let ctx = load_ctx();
        let entries = vec![diff("src/ast.rs.bog"), diff("src/cli.rs.bog")];
    ...
    fn test_skimsystem_agent_disallowed_source_files() {
        let ctx = load_ctx();
        let entries = vec![diff("src/ast.rs")];
        let violations = check_agent_permissions("code-standards-agent", &entries, &ctx);
        assert_eq!(violations.len(), 1);
    ...
    fn test_unknown_agent_all_violations() {
        ...
        assert_eq!(violations.len(), 1);
        assert!(violations[0].reason.contains("not declared"));
    }
}
