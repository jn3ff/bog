...
use bog::ast::{Annotation, ChangeRequest};
use bog::parser;
...
use crate::plan::{AgentResult, AgentResultStatus, AgentTask};
...
mod tests {
    ...
    fn test_build_task_from_requests() {
        let wp = SubsystemWorkPacket {
            ...
            requests: vec![(
                "crates/bog/src/parser.rs.bog".to_string(),
                "crates/bog/src/parser.rs".to_string(),
                vec![ChangeRequest {
                    id: "test-123".to_string(),
                    from: "code-standards-agent".to_string(),
                    target: bog::ast::Value::FnRef("parse_bog".to_string()),
        ...
        assert!(task.focus_files.contains(&"crates/bog/src/parser.rs".to_string()));
        assert!(task.focus_files.contains(&"crates/bog/src/parser.rs.bog".to_string()));
    }
}
